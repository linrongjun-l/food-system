...
import cn.ecut.lrj.modules.ums.service.*;
...
import cn.ecut.lrj.modules.ums.dto.UmsAdminLoginParam;
...
public class UmsAdminController {
    ...
    public void getKaptcha(HttpServletResponse response, HttpSession session) {
        ...
        // 将验证码存入session
        session.setAttribute(ComConstants.KAPTCHA, text);
    ...
    @ResponseBody
    public CommonResult login(@RequestBody @Validated  UmsAdminLoginParam umsAdminLoginParam,HttpSession session) {

        UmsAdmin login = adminService.login(umsAdminLoginParam.getUsername(), umsAdminLoginParam.getPassword(),umsAdminLoginParam.getKaptcha(),session);

        Map<String, String> tokenMap = new HashMap<>();


        // jwt
        String token = jwtTokenUtil2.generateToken(login.getUsername());
    ...
    public CommonResult getAdminInfo() {
        ...
        //UmsAdmin umsAdmin = adminDetail.getUmsAdmin();

        String userName = JwtTokenUtil2.currentAdmin.get();
